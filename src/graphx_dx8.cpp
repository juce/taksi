...
#define RESET_REL_POS 8
#define PRESENT_REL_POS 9
...
// debugging tools
extern int g_clockIndent;
extern list<DWORD> g_debugClock;
...
static IDirect3DSurface8* g_tempSurf;
...
BYTE g_codeFragment_p8[5] = {0, 0, 0, 0, 0};
BYTE g_codeFragment_r8[5] = {0, 0, 0, 0, 0};
BYTE g_jmp_p8[5] = {0, 0, 0, 0, 0};
BYTE g_jmp_r8[5] = {0, 0, 0, 0, 0};
DWORD g_savedProtection_p8 = 0;
DWORD g_savedProtection_r8 = 0;

// video capture parameters
extern ALGORITHM_TYPE g_algorithm;

static int g_numHeapSearches = 0;
static int g_maxHookAttempts = 2;
...
static IDirect3DDevice8* g_dev = NULL;
static IDirect3DVertexBuffer8* g_pVBidle = NULL;
static IDirect3DVertexBuffer8* g_pVBswHook = NULL;
static IDirect3DVertexBuffer8* g_pVBrec = NULL;
static IDirect3DVertexBuffer8* g_pVBborder = NULL;
static DWORD g_dwSavedStateBlock = 0L;
static DWORD g_dwDrawTextStateBlock = 0L;

/* IDirect3DDevice8 method-types */
typedef ULONG   (STDMETHODCALLTYPE *ADDREF8)(IUnknown* self);
typedef ULONG   (STDMETHODCALLTYPE *RELEASE8)(IUnknown* self);
typedef HRESULT (STDMETHODCALLTYPE *RESET8)(IDirect3DDevice8* self, LPVOID);
typedef HRESULT (STDMETHODCALLTYPE *PRESENT8)(IDirect3DDevice8* self, CONST RECT*, CONST RECT*, HWND, LPVOID);

/* function pointers */
static ADDREF8 g_D3D8_AddRef = NULL;
static RELEASE8 g_D3D8_Release = NULL;
static RESET8   g_D3D8_Reset = NULL;
static PRESENT8 g_D3D8_Present = NULL;
...
static HRESULT LoadSurfaceFromFile(IDirect3DSurface8* surf, char* filename);
static HRESULT SaveSurfaceToFile(IDirect3DSurface8* surf, char* filename, RECT* rect);
static HRESULT InvalidateDeviceObjects(IDirect3DDevice8* dev, BOOL detaching);
static HRESULT DeleteDeviceObjects(IDirect3DDevice8* dev);
static void DrawIndicator(LPVOID);
static HRESULT JuceGetRGB(IDirect3DDevice8* d3dDevice, 
IDirect3DSurface8* srcSurf, RECT* rect, D3DFORMAT format, BYTE* destBuf, INT destPitch);
static HRESULT JuceGetHalfSizeRGB(IDirect3DDevice8* d3dDevice, 
IDirect3DSurface8* srcSurf, RECT* rect, D3DFORMAT format, BYTE* destBuf, INT destPitch);

EXTERN_C ULONG _declspec(dllexport) STDMETHODCALLTYPE JuceAddRef8(
IUnknown* self);
EXTERN_C ULONG _declspec(dllexport) STDMETHODCALLTYPE JuceRelease8(
IUnknown* self);
EXTERN_C HRESULT _declspec(dllexport) STDMETHODCALLTYPE JuceReset8(
IDirect3DDevice8* self, LPVOID params);
EXTERN_C HRESULT _declspec(dllexport) STDMETHODCALLTYPE JucePresent8(
IDirect3DDevice8* self, CONST RECT* src, CONST RECT* dest, HWND hWnd, LPVOID unused);

void InitHooksD8();
static void HookDeviceMethods8(HMODULE hMod);

static HWND GetProcWnd(IDirect3DDevice8* d3dDevice);
static void GetBackBufferInfo(IDirect3DDevice8* d3dDevice);
static void MakeScreenShot(IDirect3DDevice8* d3dDevice, BYTE* rgbBuf, SIZE_T size, INT pitch);
static void MakeSmallScreenShot(IDirect3DDevice8* d3dDevice, BYTE* rgbBuf, SIZE_T size, INT pitch);
static void GetFrame(IDirect3DDevice8* d3dDevice, BYTE* rgbBuf, SIZE_T size, INT pitch);
static HRESULT InitVB(IDirect3DDevice8* dev);
static HRESULT RestoreDeviceObjects(IDirect3DDevice8* dev);

// function pointer for grabbing RGB-data
typedef HRESULT (*PFNGETPIXELSPROC)(IDirect3DDevice8*, IDirect3DSurface8*, RECT*, D3DFORMAT, BYTE*, INT);
...
/* This function hooks two IDirect3DDevice8 methods, using code overwriting technique. */
void HookDeviceMethods8(HMODULE hMod)
{
	...
	// with a JMP instruction to our routine (JucePresent8).
	...
	if (!hMod)
	{
		Log("HookDeviceMethods8: d3d8 is not loaded at this moment. No hooking performed.");
		return;
	}
	if (!g_dxInfo.present8 || !g_dxInfo.reset8)
	{
		Log("HookDeviceMethods8: No info on Present and/or Reset.");
		Log("HookDeviceMethods8: Possible reason: DirectX 8.1 is not installed.");
		return;
	}
	
	g_D3D8_Present = (PRESENT8)((DWORD)hMod + g_dxInfo.present8);
	g_D3D8_Reset = (RESET8)((DWORD)hMod + g_dxInfo.reset8);

	TRACE("HookDeviceMethods8: checking JMP-implants...");
	if (memcmp(g_D3D8_Present, g_jmp_p8, 5)==0)
	{
		TRACE("HookDeviceMethods8: Present allready has JMP-implant.");
	}
	else
	{
		TRACE2("HookDeviceMethods8: g_D3D8_Present = %08x", (DWORD)g_D3D8_Present);
		TRACE2("HookDeviceMethods8: JucePresent8 = %08x", (DWORD)JucePresent8);

		// unconditional JMP to relative address is 5 bytes.
		g_jmp_p8[0] = 0xe9;
		DWORD addr = (DWORD)JucePresent8 - (DWORD)g_D3D8_Present - 5;
		TRACE2("JMP %08x", addr);
		memcpy(g_jmp_p8 + 1, &addr, sizeof(DWORD));

		memcpy(g_codeFragment_p8, g_D3D8_Present, 5);
		DWORD newProtection = PAGE_EXECUTE_READWRITE;
		if (VirtualProtect(g_D3D8_Present, 8, newProtection, &g_savedProtection_p8))
		{
			memcpy(g_D3D8_Present, g_jmp_p8, 5);
			TRACE("HookDeviceMethods8: JMP-hook for Present planted.");
		}
	}

	if (memcmp(g_D3D8_Reset, g_jmp_r8, 5)==0)
	{
		TRACE("HookDeviceMethods8: Reset allready has JMP-implant.");
	}
	else
	{
		TRACE2("HookDeviceMethods8: g_D3D8_Reset = %08x", (DWORD)g_D3D8_Reset);
		TRACE2("HookDeviceMethods8: JuceReset8 = %08x", (DWORD)JuceReset8);

		// unconditional JMP to relative address is 5 bytes.
		g_jmp_r8[0] = 0xe9;
		DWORD addr = (DWORD)JuceReset8 - (DWORD)g_D3D8_Reset - 5;
		TRACE2("JMP %08x", addr);
		memcpy(g_jmp_r8 + 1, &addr, sizeof(DWORD));

		memcpy(g_codeFragment_r8, g_D3D8_Reset, 5);
		DWORD newProtection = PAGE_EXECUTE_READWRITE;
		if (VirtualProtect(g_D3D8_Reset, 8, newProtection, &g_savedProtection_r8))
		{
			memcpy(g_D3D8_Reset, g_jmp_r8, 5);
			TRACE("HookDeviceMethods8: JMP-hook for Reset planted.");
		}
	}
}
...
{
	if (pAddRefPtr != NULL && g_D3D8_AddRef != NULL)
		*pAddRefPtr = (DWORD)g_D3D8_AddRef;

	if (pReleasePtr != NULL && g_D3D8_Release != NULL)
		*pReleasePtr = (DWORD)g_D3D8_Release;

	Log("RestoreDeviceMethods8: done.");
}


/* determine window handle */
HWND GetProcWnd(IDirect3DDevice8* d3dDevice)
{
...
void GetBackBufferInfo(IDirect3DDevice8* d3dDevice)
{
	...
	if (SUCCEEDED(d3dDevice->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &g_backBuffer)))
	{
		...
		IUnknown* obj = (IUnknown*)g_backBuffer;
 ...
 */
HRESULT LoadSurfaceFromFile(IDirect3DSurface8* surf, char* filename)
{
	BYTE* rgbBuf = NULL;
 ...
 */
HRESULT SaveSurfaceToFile(IDirect3DSurface8* surf, char* filename, RECT* rect)
{
	D3DSURFACE_DESC desc;
	if (FAILED(surf->GetDesc(&desc)))
	{
...
void MakeScreenShot(IDirect3DDevice8* d3dDevice, BYTE* rgbBuf, SIZE_T size, INT pitch)
{
	IDirect3DSurface8* buffer;
	if (FAILED(d3dDevice->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &buffer)))
	{
...
void MakeSmallScreenShot(IDirect3DDevice8* d3dDevice, BYTE* rgbBuf, SIZE_T size, INT pitch)
{
	IDirect3DSurface8* buffer;
	if (FAILED(d3dDevice->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &buffer)))
	{
...
/* Makes a small screen shot (1/2 original size) from back buffer. Doesn't use D3DX functions.*/
void GetFrame(IDirect3DDevice8* d3dDevice, BYTE* rgbBuf, SIZE_T size, INT pitch)
{
	IDirect3DSurface8* buffer;
	if (FAILED(d3dDevice->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &buffer)))
	{
...
{
	TRACE("#################################");
	TRACE("JuceReset8: called.");
	Log("JuceReset8: cleaning-up.");

	BYTE* codeDest = (BYTE*)g_D3D8_Reset;

	// put back saved code fragment
	codeDest[0] = g_codeFragment_r8[0];
	*((DWORD*)(codeDest + 1)) = *((DWORD*)(g_codeFragment_r8 + 1));
	...
	TRACE("JuceReset8: Reset() is done. About to return.");

	// put JMP instruction again
	codeDest[0] = g_jmp_r8[0];
	*((DWORD*)(codeDest + 1)) = *((DWORD*)(g_jmp_r8 + 1));

	return res;
}

/* New AddRef function */
EXTERN_C ULONG _declspec(dllexport) STDMETHODCALLTYPE JuceAddRef8(IUnknown* self)
{
	g_refCount = g_D3D8_AddRef(self);
	TRACE2("JuceAddRef8: called (g_refCount = %d).", g_refCount);
	return g_refCount;
}

/* New Release function */
EXTERN_C ULONG _declspec(dllexport) STDMETHODCALLTYPE JuceRelease8(IUnknown* self)
{
	// a "fall-through" case
	if ((g_refCount > g_myCount + 1) && g_D3D8_Release) 
	{
		g_refCount = g_D3D8_Release(self);
		TRACE2("JuceRelease8: called (g_refCount = %d).", g_refCount);
		return g_refCount;
	}

	TRACE("+++++++++++++++++++++++++++++++++++++");
	TRACE("JuceRelease8: called.");
	TRACE2("JuceRelease8: self = %08x", (DWORD)self);
	TRACE2("JuceRelease8: VTABLE[0] = %08x", ((DWORD*)(*((DWORD*)self)))[0]);
	TRACE2("JuceRelease8: VTABLE[1] = %08x", ((DWORD*)(*((DWORD*)self)))[1]);
	TRACE2("JuceRelease8: VTABLE[2] = %08x", ((DWORD*)(*((DWORD*)self)))[2]);
	...
	TRACE("JuceRelease8: about to call real Release.");
	g_refCount = g_D3D8_Release(self);
	TRACE("JuceRelease8: real Release done.");
	TRACE2("JuceRelease8: g_refCount = %d", g_refCount);

	TRACE("JuceRelease8() done.");
...
{
	TRACE("--------------------------------");
	TRACE("JucePresent8: called.");
	/* log the usage */
	//Log("JucePresent8: Present() is about to be called.");
	
	BYTE* codeDest = (BYTE*)g_D3D8_Present;

	// put back saved code fragment
	codeDest[0] = g_codeFragment_p8[0];
	*((DWORD*)(codeDest + 1)) = *((DWORD*)(g_codeFragment_p8 + 1));
	...
	if (pAddRefPtr == NULL || pReleasePtr == NULL)
	{
		...
		// hook AddRef method
		g_D3D8_AddRef = (RELEASE8)(*pAddRefPtr);
		*pAddRefPtr = (DWORD)JuceAddRef8;

		// hook Release method
		g_D3D8_Release = (RELEASE8)(*pReleasePtr);
		*pReleasePtr = (DWORD)JuceRelease8;
	}
	...
	if (!g_mystate.bKeyboardInitDone)
	{
		if (g_config.useDirectInput)
		{
			...
			if (hDI)
			{
				TRACE("JucePresent8: KEYBOARD INIT: using DirectInput.");
			...
			{
				TRACE("JucePresent8: KEYBOARD INIT: Unable to load dinput8.dll.");
			}
		}
		...
		if (!g_mystate.bKeyboardInitDone)
		{
			TRACE("JucePresent8: KEYBOARD INIT: using keyboard hook.");
		...
		{
			Log("JucePresent8: Signaling to unhook GetMsgProc.");
			if (UninstallTheHook())
			{
				Log("JucePresent8: GetMsgProc unhooked.");
	...
	{
		TRACE("JucePresent8: calling OpenAVIFile.");
	...
	{
		TRACE("JucePresent8: calling CloseAVIFile.");
		CloseAVIFile(videoFile);
	...
	/* check unhook flag */
	//TRACE2("JucePresent8: sg_bUnhookDevice = %d.", GetUnhookFlag());
	if (GetUnhookFlag())
	{
		// we're ordered to unhook IDirect3DDevice8 methods.
		RestoreDeviceMethods8();
	...
	{
		Log("JucePresent8: Reconfiguring custom settings...");
	...
	TRACE("JucePresent8: Present() is done. About to return.");

	// put JMP instruction again
	codeDest[0] = g_jmp_p8[0];
	*((DWORD*)(codeDest + 1)) = *((DWORD*)(g_jmp_p8 + 1));
...
HRESULT InitVB(IDirect3DDevice8* dev)
{
	...
	if (FAILED(dev->CreateVertexBuffer(4*sizeof(CUSTOMVERTEX), D3DUSAGE_WRITEONLY, 
					D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &g_pVBidle)))
	{
	...
	if (FAILED(dev->CreateVertexBuffer(4*sizeof(CUSTOMVERTEX), D3DUSAGE_WRITEONLY, 
					D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &g_pVBswHook)))
	{
		Log("CreateVertexBuffer() failed.");
		return E_FAIL;
	}
	...
	if (FAILED(dev->CreateVertexBuffer(4*sizeof(CUSTOMVERTEX), D3DUSAGE_WRITEONLY, 
					D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &g_pVBrec)))
	{
		Log("CreateVertexBuffer() failed.");
		return E_FAIL;
	}
	...
	if (FAILED(dev->CreateVertexBuffer(5*sizeof(CUSTOMVERTEX), D3DUSAGE_WRITEONLY, 
					D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &g_pVBborder)))
	{
		Log("CreateVertexBuffer() failed.");
		return E_FAIL;
	}
...
HRESULT RestoreDeviceObjects(IDirect3DDevice8* dev)
{
	...
	for( UINT which=0; which<2; which++ )
	{
		...
		dev->SetStreamSource( 0, g_pVBidle, sizeof(CUSTOMVERTEX) );
...
HRESULT InvalidateDeviceObjects(IDirect3DDevice8* dev, BOOL detaching)
{
	...
	SAFE_RELEASE2( g_tempSurf, g_myCount );
	...
	TRACE("InvalidateDeviceObjects: done.");
    return S_OK;
}
...
void DrawIndicator(LPVOID self)
{
	...
	if (g_mystate.bNowRecording) dev->SetStreamSource( 0, g_pVBrec, sizeof(CUSTOMVERTEX) );
	else if (g_mystate.bSystemWideMode) dev->SetStreamSource( 0, g_pVBswHook, sizeof(CUSTOMVERTEX) );
	else dev->SetStreamSource( 0, g_pVBidle, sizeof(CUSTOMVERTEX) );
	dev->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, 2 );

	dev->SetStreamSource( 0, g_pVBborder, sizeof(CUSTOMVERTEX) );
...
HRESULT JuceGetRGB(IDirect3DDevice8* d3dDevice, 
IDirect3DSurface8* srcSurf, RECT* rect, D3DFORMAT format, BYTE* destBuf, INT destPitch)
{
	...
	if (g_tempSurf == NULL)
	{
		if (FAILED(d3dDevice->CreateImageSurface(width, height, format, &g_tempSurf)))
		{
	...
	IDirect3DSurface8* surf = g_tempSurf;
	...
	if (FAILED(d3dDevice->CopyRects(srcSurf, rect, 1, surf, points)))
	{
		Log("JuceGetRGB: CopyRects() failed for image surface.");
		return E_FAIL;
	}
...
/* Copies pixel's RGB data into dest buffer. Alpha information is discarded. */
HRESULT JuceGetHalfSizeRGB(IDirect3DDevice8* d3dDevice, 
IDirect3DSurface8* srcSurf, RECT* rect, D3DFORMAT format, BYTE* destBuf, INT destPitch)
{
	...
	if (FAILED(d3dDevice->CopyRects(srcSurf, rect, 1, surf, points)))
	{
		Log("JuceGetHalfSizeRGB: CopyRects() failed for image surface.");
		return E_FAIL;
	}
	CLOCK_STOP_AND_LOG("CopyRects:          %10d", g_clockIndent, g_debugClock);
	...
	CLOCK_STOP_AND_LOG("LockRect:           %10d", g_clockIndent, g_debugClock);
	...
	CLOCK_STOP_AND_LOG("bit transform:      %10d", g_clockIndent, g_debugClock);

	// unlock rect
	surf->UnlockRect();
	//Log("Rect unlocked.");
	
	CLOCK_STOP_AND_LOG("JuceGetHalfSizeRGB time: %10d", g_clockIndent, g_debugClock);
	return S_OK;
}
...
void InitHooksD8()
{
	//MasterLog(g_myinfo.logfile, g_myinfo.processfile, " uses Direct3D8");
	
	// set function pointer to capture frame for video
	GetPixels = (g_config.fullSizeVideo) ? JuceGetRGB : JuceGetHalfSizeRGB;

	// Initialize logging and config params, if not already done.
	if (!g_initDone)
	{
		...
		LogWithNumber("g_config.fullSizeVideo = %d", g_config.fullSizeVideo);
...
void GraphicsCleanup8()
{
	...
	if (g_D3D8_Present != NULL && g_codeFragment_p8[0] != 0)
	{
		try 
		{
			memcpy(g_D3D8_Present, g_codeFragment_p8, 5);
			VirtualProtect(g_D3D8_Present, 8, g_savedProtection_p8, &g_savedProtection_p8);
		}
		catch (...)
		{
			TRACE("GraphicsCleanup8: problem restoring Present code fragment.");
		}
	}
	if (g_D3D8_Reset != NULL && g_codeFragment_r8[0] != 0)
	{
		try 
		{
			memcpy(g_D3D8_Reset, g_codeFragment_r8, 5);
			VirtualProtect(g_D3D8_Reset, 8, g_savedProtection_r8, &g_savedProtection_r8);
		}
		catch (...)
		{
			TRACE("GraphicsCleanup8: problem restoring Reset code fragment.");
		}
	}
	if (g_D3D8_AddRef != NULL && pAddRefPtr != NULL)
	{
		*pAddRefPtr = (DWORD)g_D3D8_AddRef;
	}
	if (g_D3D8_Release != NULL && pReleasePtr != NULL)
	{
		*pReleasePtr = (DWORD)g_D3D8_Release;
	}
	...
	TRACE("GraphicsCleanup8: done.");
}

// verifies that JMP-implants are still in place.
void VerifyD8()
{
	HookDeviceMethods8(GetModuleHandle("d3d8.dll"));
}
